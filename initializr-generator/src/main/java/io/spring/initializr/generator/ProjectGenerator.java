...
public class ProjectGenerator {
	...
	/**
	 * 生成工程目录
	 * @param codeDir 目标代码目录
	 * @param resourceTarget 目标资源目录
	 * @param model 依赖信息
	 */
	private void generateDirByArchetype(File codeDir, File resourceTarget, Map<String, Object> model) {
		try {
			URL archetypeUrl = Thread.currentThread().getContextClassLoader().getResource("templates/archetype");
//			log.info("模板源目录url:"+ archetypeUrl.toString());
//			log.info("java目标目录:" + codeDir.getCanonicalPath());
//			log.info("resources目标目录:" + resourceTarget.getCanonicalPath());

			// 编译器中运行
			if ("file".equals(archetypeUrl.getProtocol())) {
				String archetypePath = URLDecoder.decode(archetypeUrl.getFile(), "UTF-8");
				// java目录拷贝
				copyDirInDev(new File(archetypePath, "java"), codeDir, model);
				// resources目录拷贝
				copyDirInDev(new File(archetypePath, "resources"), resourceTarget, model);
			}
			// 打成jar包部署
			else if ("jar".equals(archetypeUrl.getProtocol())) {
				// java目录拷贝
				copyDirInJar(archetypeUrl, codeDir, "templates/archetype/java/", model);
				// resource目录拷贝
				copyDirInJar(archetypeUrl, resourceTarget, "templates/archetype/resources", model);
			}
		} catch (Exception e) {
	...
	/**
	 * 以Jar包形式部署时拷贝工程目录
	 * @param archetypeUrl resources目录在Jar包中的url
	 * @param targetDir 目标目录
	 * @param sourceDir 模板源目录
	 * @param model 项目数据
	 */
	private void copyDirInJar(URL archetypeUrl, File targetDir, String sourceDir, Map<String, Object> model) {
		try {
			JarFile jar = ((JarURLConnection) archetypeUrl.openConnection()).getJarFile();
			Enumeration<JarEntry> entries = jar.entries();
			// 遍历 initializr-generator.jar 包下的resources目录
			while (entries.hasMoreElements()) {
				JarEntry entry = entries.nextElement();
				String entryName = entry.getName();
				// 如果不是要找的模板目录则跳过
				if (!entryName.contains(sourceDir)) {
					continue;
				}
				if (entry.isDirectory()) {
					String srcDir = entryName.replace(sourceDir, "");
					File targetNewDir = new File(targetDir, srcDir);
					if (!targetNewDir.exists()) {
						targetNewDir.mkdirs();
					}
					continue;
				}
				// 模板文件名，从templates/开始
				String sourceTemplateName = entryName.replace("templates/", "");
//				log.info("模板文件名: " + sourceTemplateName);
//				log.info("目标路径: " + entryName.replace(sourceDir, ""));
				// 渲染模板文件到临时目录
				write(new File(targetDir, entryName.replace(sourceDir, "")), sourceTemplateName, model);
			}
		} catch (Exception e) {
			log.error(e.getMessage());
		}
	 ...
	 * @param model 项目数据
	 */
	private void copyDirInDev(File source, File target, Map<String, Object> model) {
		if (!source.exists()) {
			return;
		}
		if (source.isDirectory()) {
			...
			for (File file : source.listFiles()) {
				File next_source = new File(source, file.getName());
				File next_target = new File(target, file.getName());
				copyDirInDev(next_source, next_target, model);
			}
		} else if (source.isFile()) {
			// 模板文件路径需从 /resources/templates/ 开始
			String templatePath = source.getAbsolutePath().split("/templates/")[1];
			write(target, templatePath, model);
		}
	}
